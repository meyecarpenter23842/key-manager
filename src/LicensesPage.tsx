    ...
    <div className="page">
      ...
      <section className="panel">
        ...
        <div className="table-wrap">
          <table><thead><tr><th>License</th><th>Ứng dụng</th><th>Khách hàng</th><th>Thiết bị</th><th>Hết hạn</th><th>Trạng thái</th><th /></tr></thead><tbody>{loading ? <LoadingRows columns={7} /> : items.map((item) => <tr key={item.id} className="clickable-row" onClick={() => setSelected(item.id)}><td><div className="cell-stack"><strong className="mono">{item.licenseKeyPreview}</strong><small>{item.licenseType === "LIFETIME" ? "Vĩnh viễn" : "Có thời hạn"}</small></div></td><td><div className="cell-title"><ApplicationAvatar appCode={item.appCode} iconDataUrl={item.applicationIconDataUrl} /><div><strong>{item.applicationName}</strong><small>{item.appCode}</small></div></div></td><td><div className="cell-stack"><strong>{item.customerName || "—"}</strong><small>{item.customerCompany || item.customerEmail || "Chưa gán khách"}</small></div></td><td><span className="device-count"><MonitorIcon size={15} /> {item.activeDeviceCount}/{item.maxDevices}</span></td><td>{formatExpiry(item.expiresAt)}</td><td><StatusBadge status={item.status} /></td><td><button className="icon-button small" type="button" aria-label="Chi tiết"><MoreIcon size={17} /></button></td></tr>)}</tbody></table>
